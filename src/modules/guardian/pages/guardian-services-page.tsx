...
function AutoCallSheet({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    const handleRequestError = (error: unknown) => {
        ...
        // ٤٢٩ رسالتُها من Laravel إنجليزيّةٌ محضة («Too Many Attempts.») ولا
        // تقول للواقف عند البوّابة ما يفعل. والحدُّ عشرون طلباً في الدقيقة —
        // من بلغه ضاغطٌ متكرّرٌ قلق، والفعلُ المطلوب أن ينتظر لا أن يعيد.
        const statusCode = getStatusCode(error)
        const message =
            statusCode === 401
                ? 'انتهت جلستك — أعد الدخول إلى البوابة ثم حاول مرة أخرى.'
                : statusCode === 429
                    ? 'محاولات كثيرة متتالية — انتظر دقيقة ثم حاول مرة أخرى.'
                    : getErrorMessage(error, 'تعذّر إرسال طلب المناداة، حاول مرة أخرى.')
        setServerRejection(message)
        toast({ type: 'error', title: message })

        // حلقةٌ مغلقةٌ كان لا مخرج منها: حين تفشل قراءةُ الإعدادات لا تعرف
        // البوّابةُ أنّ للمدرسة سياجاً، فلا تقيس الموقع (المُقيس محروسٌ بوجود
        // السياج)، فترسل الطلبَ بلا إحداثيّات، فيرفضه الخادمُ بـ«يجب تفعيل
        // خدمة الموقع» — ووليُّ الأمر يرى أنّ الموقع مفعَّلٌ في جهازه فيعيد
        // الضغط، فيُرفض ثانيةً، إلى ما لا نهاية.
        //
        // فحين يأتي رفضٌ من الخادم ولا موقعَ في يدنا، نقيسه: المحاولةُ التالية
        // تحمله وتنجح.
        if (statusCode === 400 && !gate.coords) {
