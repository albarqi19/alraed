...
/* ═══════════════════════════════════════════════════════════
   ...
   الملفُ غِطاءٌ يُلقى على السجل. ما غطّاه بقي، وما لم يغطّه نام.
   هذه الصفحة وحدها في النظام تُعطّل بالإغفال — بأن لا يذكرك ملفٌ —
   فالهندسة ترسم قِصَر الغطاء لا رقماً أحمر.

   كان هذا حذفاً حتى أُصلح: `Student` بلا SoftDeletes وواحد وعشرون جدولاً
   معلَّقٌ عليه بـ CASCADE، فكانت ضغطةٌ واحدة عند بداية العام تمحو سجلّ كل
   مغادر في السنوات كلها. صار تعطيلاً، والنبرة هنا تبعته: كهرمانيّ لا أحمر،
   و«نام» لا «سقط». التحذير الذي يبالغ يُدرَّب المستخدم على تجاهله.
   ═══════════════════════════════════════════════════════════ */
      ...
      >
        ...
        <p style={{ margin: '8px 0 0', fontSize: 10.5, fontVariantNumeric: 'tabular-nums' }}>
          {c.D === 0 ? (
            ...
            <span style={{ color: TONES.amber.tx }}>
              سيُعطَّل {c.D} اسماً
              {c.seats > 0 && ` — و${c.seats.toLocaleString('ar-SA-u-nu-latn')} صفاً في الجداول يبقى محفوظاً في الأرشيف`}
              {c.care > 0 && `، منها ${c.care} ملف إرشاد`}. يعودون بذكرهم في ملفٍ قادم.
            </span>
          ) : (
