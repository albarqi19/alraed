...
import {
  fetchStudentReport,
  type StudentReportSectionKey,
  type StudentReportSignatureKey,
} from '@/services/api/student-report'
import { printStudentReport } from '../utils/print-student-report'
import type { StudentRecord } from '../types'

interface StudentReportPrintDialogProps {
...
const SECTION_OPTIONS: Array<{ key: StudentReportSectionKey; label: string }> = [
  { key: 'absences', label: 'الغياب' },
  { key: 'leave_requests', label: 'الاستئذان' },
  { key: 'lates', label: 'التأخيرات' },
  { key: 'behavior_violations', label: 'المخالفات السلوكية' },
  { key: 'academic_weakness', label: 'الضعف الدراسي' },
  { key: 'referrals', label: 'الإحالات' },
]

const SIGNATURE_OPTIONS: Array<{ key: StudentReportSignatureKey; label: string }> = [
  { key: 'parent', label: 'ولي الأمر' },
  { key: 'deputy', label: 'الوكيل' },
  { key: 'principal', label: 'مدير المدرسة' },
]

const ALL_SECTIONS_TRUE: Record<StudentReportSectionKey, boolean> = {
  absences: true,
  leave_requests: true,
  lates: true,
  behavior_violations: true,
  academic_weakness: true,
  referrals: true,
}

const DEFAULT_SIGNATURES: Record<StudentReportSignatureKey, boolean> = {
  parent: true,
  deputy: false,
  principal: true,
}
...
export function StudentReportPrintDialog({
  student,
  open,
  onOpenChange,
}: StudentReportPrintDialogProps) {
  const [periodMode, setPeriodMode] = useState<PeriodMode>('semester')
  const [selectedSemesterId, setSelectedSemesterId] = useState<number | null>(null)
  const [customRange, setCustomRange] = useState<{ start: string; end: string }>({ start: '', end: '' })
  const [sections, setSections] = useState<Record<StudentReportSectionKey, boolean>>({ ...ALL_SECTIONS_TRUE })
  const [signatures, setSignatures] = useState<Record<StudentReportSignatureKey, boolean>>({ ...DEFAULT_SIGNATURES })
  ...
  const toggleSection = (key: StudentReportSectionKey) =>
    setSections((prev) => ({ ...prev, [key]: !prev[key] }))

  const toggleSignature = (key: StudentReportSignatureKey) =>
  ...
  }, [student, selectedSectionKeys.length, periodMode, selectedSemesterId, customRange.start, customRange.end])
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const data = await fetchStudentReport(student.id, params)
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto" dir="rtl">
        <DialogHeader>
          <DialogTitle className="text-right">طباعة تقرير شامل لملف الطالب</DialogTitle>
          <DialogDescription className="text-right">
            {student ? `الطالب: ${student.name}` : 'اختر الفترة والبنود ثم اضغط طباعة.'}
          </DialogDescription>
        </DialogHeader>
